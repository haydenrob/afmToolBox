...
class AFM():
    ...
    def read_const(self, data):
        ...
        self.DeflSens = float(re.split('(\d+)', ln)[1] + '.' + re.split('(\d+)', ln)[3])

        if self.SprCons == 0:
            ln = re.findall(r'\Spring Constant: .*',line)[0]
            self.SprCons = float(re.split('(\d+)', ln)[1] + '.' + re.split('(\d+)', ln)[3])

        ln = re.findall(r'\@4:Z scale: .*',line)[0]
        self.zScale = float(re.split('(\d+)', ln)[3] + '.' + re.split('(\d+)', ln)[5])
    ...
    def read_data(self, f):
        ...
        try:
            data = pd.read_csv(f, skiprows=self.start_data - 1, sep='\t', usecols = [0,1,2,3], encoding = "ISO-8859-1")
        except:
            AFM.find_start(self)
            data = pd.read_csv(f, skiprows=self.start_data - 1, sep='\t', usecols = [0,1,2,3], encoding = "ISO-8859-1")
        ...
        data_ret = data_ret.rename(columns={"Calc_Ramp_Ex_nm": "z_nm", "Defl_V_Ex": "defl_V"})
        data_app = data_app.rename(columns={"Calc_Ramp_Rt_nm": "z_nm", "Defl_V_Rt": "defl_V"})
    ...
    def x_cal(self, i_skp = 10, e_skp = 10, plot = False):
        ...
        for idx, fh in enumerate(self.data):
            ...
            for i in range(len(y_data[i_skp:])):
                r2 = np.corrcoef(z_data[i_skp:i+i_skp+2].astype(float), y_data[i_skp:i+i_skp+2].astype(float))[0,1]**2
                if r2 < 0.99:
                    break
            ...
            try:
                ...
                if not flip: #only want to append it once, not both times for app and ret
    ...
    def final_pulloff(self):
        ...
        for i in range(len(yn)):
            ...
            for s in range(len(grads)):
                grads[s] = (yn[i+s+1] - yn[i+s]) / (xn[i+s] - xn[i+s+1])
            ...
            if (av < 0) & (grads[-1] < grads[0]) & (grads[-2] < grads[0]) & (grads[-3] < grads[0]) & (grads[-4] < grads[0]):
                ...
                if abs(np.mean(y[y_start:y_start + 4])) > 0.05:
                    print(f'didnt snap back to baseline: {y[y_start]:.03f}', self.f)
                    self.bad = True
                    # self.stop = True
    ...
    def wlc(x, L_p, L_c, T):
        return ((k_B * T) / L_p) * ((x / L_c) - (1 / (4 * (1 - (x / L_c))**2)) - (1 / 4))
    ...
    def auto_params(self, L_p = 0.8e-9, L_c = 300e-9):
        ...
        self.Lp_min = 0.1e-9 #TODO shouldnt be too much less than monomer legnth... 
        self.Lp_max = 3e-9
    ...
    def wlc_fit(self, params, plot = True, method = 'differential_evolution', show_plot = False):
        """
        ...
        op = lmfit.minimize(AFM.resid, params, args=(x_data, y_data), method = method)
        self.chisqr = op.chisqr

        x1 = np.array(x_data)[0]
        x2 = np.array(x_data)[-1]
        y1 = AFM.wlc(x1, op.params['L_p'].value, op.params['L_c'].value, op.params['T'].value)
        y2 = AFM.wlc(x2, op.params['L_p'].value, op.params['L_c'].value, op.params['T'].value)
        ...
        if plot:
            fig, ax = plt.subplots()
            ax.plot(x_data.values * 1e9, np.array(AFM.wlc(x_data, op.params['L_p'].value, op.params['L_c'].value,  op.params['T'].value)) * 1e9)
            
            x = self.data[1]['z_cal_nm'][self.y_end-100:self.y_start+100].values
            y = self.data[1]['defl_cal_nN'][self.y_end-100:self.y_start+100].values
            ax.plot(x, y)
    ...
    def radius_gyration(self):
        ...
        R_g = np.sqrt((25 / 176) * (self.a**2) * (self.L_c / self.a)**(6/5))
    ...
    def save_data(self, path, plot = True):
        ...
        if plot:
            fig, ax = plt.subplots()
            ax.plot(self.data[1]['z_cal_nm'].values, self.data[1]['defl_cal_nN'].values, label = 'Retract')
            ax.plot(self.data[0]['z_cal_nm'].values, self.data[0]['defl_cal_nN'].values, label = 'Approach')
    ...
    def batch_process(self, files, update = False, WLC = False, method = 'differential_evolution'):
        done = False
        ...
        # while not AFM.stop:
            ...
            if WLC:
                ...
                if not self.stop:
                    ...
                    if (self.L_p > tol_ub * self.Lp_max) or (self.L_p < tol_lb * self.Lp_min):
                        self.bad = True

                    if (self.L_c > tol_ub * self.Lc_max) or (self.L_c < tol_lb * self.Lc_min):
                        self.bad = True
                    ...
                    to_append = {'run_number' : self.f[:-4], 
                                ...
                                'Rg' : self.R_g,
                                'Sigma' : self.Sigma,
                                'Chain_density' : 1/np.sqrt(self.sigma) # : 1 chain per ___ nm
                                }
                    
                    wlc_summary = wlc_summary.append(to_append, ignore_index = True)

            wlc_summary.to_csv(fh, index = False)

    
        print(f'{len(AFM.fc)} curves processed and {len(AFM.not_fc)} curves discarded.')


    def plot_raw(self):
        fig, ax = plt.subplots()
        ax.plot(self.data[1]['z_nm'].values, self.data[1]['defl_V'].values, label = 'Retract')
        ax.plot(self.data[0]['z_nm'].values, self.data[0]['defl_V'].values, label = 'Approach')
